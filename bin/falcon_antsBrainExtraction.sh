...
EXTRACTION_BRAIN_TMP=${TEMPDIR}/Brain.${OUTPUT_SUFFIX}
...
#                 exe_initial_align="${exe_initial_align} ${EXTRACTION_REGISTRATION_MASK}"
      ...
      if [[ ! -f ${EXTRACTION_SEGMENTATION} ]]; 
       ...
       # Pad image here to avoid errors from dilating into the edge of the image
        ...
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_WM_TMP} GetLargestComponent ${EXTRACTION_WM}
        logCmd mv ${EXTRACTION_WM_TMP} ${EXTRACTION_WM}
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_GM_TMP} GetLargestComponent ${EXTRACTION_GM}
        ...
        for img in ${EXTRACTION_SEGMENTATION} ${EXTRACTION_MASK} ${EXTRACTION_WM} ${EXTRACTION_GM} ${EXTRACTION_CSF} ${EXTRACTION_MASK_PRIOR_WARPED}
          do
            logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${TEMPDIR}/unpad.${OUTPUT_SUFFIX} PadImage ${img} -$padVoxels
            logCmd mv ${TEMPDIR}/unpad.${OUTPUT_SUFFIX} ${img}
          done

   
        logCmd ${ANTSPATH}/MultiplyImages ${DIMENSION} ${N4_CORRECTED_IMAGES[0]} ${EXTRACTION_MASK} ${EXTRACTION_BRAIN_TMP}

        # Copy header information from original image into output
        logCmd ${ANTSPATH}/CopyImageHeaderInformation ${ANATOMICAL_IMAGES[0]} ${EXTRACTION_BRAIN_TMP} ${EXTRACTION_BRAIN} 1 1 1 0
        logCmd mv  ${EXTRACTION_MASK}  ${EXTRACTION_MASK_TMP}
        logCmd ${ANTSPATH}/CopyImageHeaderInformation ${ANATOMICAL_IMAGES[0]} ${EXTRACTION_MASK_TMP}  ${EXTRACTION_MASK} 1 1 1 0
