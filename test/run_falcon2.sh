...
#niikmath=${NIIKDIR}/bin/niikmath
niikmath=${BINDIR}/niikmath
...
run_cmd itk_resample --labels --byte ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_deep2_4_cerebellum_dilate.mnc \
...
run_cmd $niikmath maskout -in=$stx_brain -mask=${ffn}_cerebellum_mask.mnc -out=${ffn}_cerebral_brain_mask.mnc -uint8


echo "white matter segmentation"
WMmask=${ffn}_WM_mask
run_cmd minccalc  $stx_cls -express 'abs(A[0]-3)<0.5?1:0'  ${WMmask}_tmp1.mnc -clobber
run_cmd $niikmath maskimg -in=${WMmask}_tmp1.mnc -mask=${stx_brain} -out=${WMmask}_tmp2.mnc 
...
run_cmd itk_resample --byte --labels ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_CLADA_fill_mask.mnc ${ffn}_FALCON_fill_mask_tmp1.mnc --like ${img} --transform ${nl_xfm} --invert --clobber
...
run_cmd itk_resample --byte --labels ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_CLADA_remove_mask.mnc ${ffn}_FALCON_remove_mask_tmp1.mnc --like ${img} --transform ${nl_xfm} --invert --clobber
...
run_cmd itk_resample --byte --labels ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_CLADA_brainstem.mnc ${ffn}_brainstem_mask_tmp1.mnc --like ${img} --transform ${nl_xfm} --invert --clobber
...
echo "  seedfill"
C=`$niikmath world2voxel-int -in=${img} -xyz=0,14,9 | tail -n1 | awk '{print $2 "," $3 "," $4}'`
run_cmd $niikmath seedfill -in=${ffn}_GWI_mask_tmp1.mnc -out=${ffn}_GWI_mask_tmp2.mnc -ijk=${C} 
echo "  close holes"
run_cmd $niikmath closeholes -in=${ffn}_GWI_mask_tmp2.mnc -out=${ffn}_GWI_mask_tmp3.mnc 

echo "  close holes"
run_cmd $niikmath median -in=${ffn}_GWI_mask_tmp3.mnc -out=${ffn}_GWI_mask_tmp4.mnc -radius=1.02 
...
echo "non-ctx mask"
run_cmd itk_resample --byte --labels ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_CLADA_nonctx_mask_2mm.mnc ${ffn}_FALCON_nonctx_mask.mnc --like ${img} --invert --clobber

echo "splitting into left/right"

run_cmd mincresample -byte ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_CLADA_midsagittal_plane_CC4.mnc ${ffn}_FALCON_split_tmp1_.mnc -like ${img} -transform $nl_xfm -invert -clobber
...
for n in `seq 0 1`; do 
  ...
  C=`$niikmath world2voxel-int -in=${img}  -xyz=24,-17,24 | tail -n1 | awk '{print $2 "," $3 "," $4}'`
  run_cmd $niikmath seedfill -in=${ffn}_GWI_mask_rt_tmp1_${n}.mnc -out=${ffn}_GWI_mask_rt_tmp2_${n}.mnc -ijk=${C} 
  run_cmd $niikmath thresh   -in=${ffn}_GWI_mask_rt_tmp2_${n}.mnc -out=${ffn}_GWI_mask_rt_tmp3_${n}.mnc -thresh=0.5 
  ...
  C=`$niikmath world2voxel-int -in=${img} -xyz=-24,-17,24 | tail -n1 | awk '{print $2 "," $3 "," $4}'`
  run_cmd $niikmath seedfill -in=${ffn}_GWI_mask_rt_tmp1_${n}.mnc -out=${ffn}_GWI_mask_lt_tmp2_${n}.mnc -ijk=${C} 
  run_cmd $niikmath thresh -in=${ffn}_GWI_mask_lt_tmp2_${n}.mnc -out=${ffn}_GWI_mask_lt_tmp3_${n}.mnc -thresh=0.5 
...
run_cmd $niikmath bounds-color -in=${img} -imglist=${ffn}_brainstem_mask.mnc,${ffn}_FALCON_remove_mask.mnc,${ffn}_FALCON_fill_mask.mnc,${wmmask},${ventmask},${ffn}_cerebellum_mask.mnc,${stx_brain},${leftmask},${rightmask} -out=${ffn}_check_seg.nii.gz -vlist=210,30,210,90,60,255,60,90,255,255,120,80,220,150,150,120,120,255,60,255,60,180,255,20,255,180,20 
...
for hemi in lt rt; do 
  ...
  if [ -e ${ffn}_GWI_mask_init_ics_${hemi}.off ]; then continue; fi
  run_cmd $niikmath closebrain -in=${ffn}_GWI_mask_${hemi}.mnc -out=${ffn}_GWI_mask_${hemi}_close.mnc -radius=10.5 
  run_cmd $niikmath dilate -in=${ffn}_GWI_mask_${hemi}_close.mnc -out=${ffn}_GWI_mask_${hemi}_dilate.mnc -radius=2 
  
  run_cmd $niikmath laplacemap -imglist=${ffn}_GWI_mask_${hemi}.mnc,${ffn}_GWI_mask_${hemi}_dilate.mnc -out=${ffn}_GWI_${hemi}_Laplace_map.mnc -xyz=0.5,0.5,0.5 
  
  # not actually used!
  # run_cmd $niikmath shrinkwrap -mask=${ffn}_GWI_mask_${hemi}_dilate.mnc -out=${ffn}_GWI_mask_${hemi}_dilate_obj.off -iter=10 -radius=35 -val=10 

  #run_kmd xfm2niikmat ${ffn}_stx.xfm $img $stximg ${ffn}_stx.mat 
  #run_kmd niikmath applyaffine-obj -obj=${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.off -out=${ffn}_init_ics_${hemi}_from_icbm.off
  # copy all components
#   run_cmd cp ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.off ${ffn}_init_ics_${hemi}_from_icbm.off
#   run_cmd cp ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.offe ${ffn}_init_ics_${hemi}_from_icbm.offe
#   run_cmd cp ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.offsp ${ffn}_init_ics_${hemi}_from_icbm.offsp


### Initialize using ICBM template
#  run_cmd ${BINDIR}/transform_off ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.off $nl_xfm  ${ffn}_init_ics_${hemi}_from_icbm.off --invert_transform --clob
#  run_cmd ${BINDIR}/niikcortex_initics ${img} ${ffn}_GWI_mask_${hemi}.mnc ${ffn}_GWI_${hemi}_Laplace_map.mnc ${ffn}_init_ics_${hemi}_from_icbm.off ${ffn}_GWI_mask_init_ics_${hemi}.off
###
    run_cmd ${BINDIR}/falcon_cortex_shrinkwrap ${ffn}_GWI_mask_${hemi}_dilate.mnc ${ffn}_GWI_mask_${hemi}_dilate2.off --iter 50  --val 10 --elen 2 --clob #-radius=35
    run_cmd ${BINDIR}/falcon_cortex_shrinkwrap ${ffn}_GWI_mask_${hemi}_dilate.mnc ${ffn}_GWI_mask_${hemi}_dilate.off  --iter 10  --val 2 --elen 1 --clob --obj=${ffn}_GWI_mask_${hemi}_dilate2.off
    run_cmd ${BINDIR}/niikcortex_initics ${img} ${ffn}_GWI_mask_${hemi}.mnc ${ffn}_GWI_${hemi}_Laplace_map.mnc ${ffn}_GWI_mask_${hemi}_dilate.iff ${ffn}_GWI_mask_init_ics_${hemi}.off

done

run_cmd $niikmath combine-obj -objlist=${ffn}_GWI_mask_init_ics_lt.off,${ffn}_GWI_mask_init_ics_rt.off -out=${ffn}_GWI_mask_init_ics.off
...
run_cmd $niikmath niikcortex-initocs -in=${img} \
      -imglist=${ffn}_cerebral_brain_mask.mnc,${ventmask},${gwimask} \
      -obj=${ffn}_GWI_mask_init_ics.off -out=${ffn}_GWI_mask_init_ocs.off
...
echo "deform both surfaces"

run_cmd ${BINDIR}/niikcortex_deform ${img} ${ffn}_cerebral_brain_mask.mnc \
                  ${ventmask} ${gwimask} \
                  ${ffn}_cerebellum_mask.mnc \
                  ${ffn}_brainstem_mask.mnc \
                  ${ffn}_GWI_mask_init_ics.off \
                  ${ffn}_GWI_mask_init_ocs.off \
                  ${OUTPUT}_ics.off ${OUTPUT}_ocs.off \
                  -nonctx-mask ${ffn}_FALCON_nonctx_mask.mnc \
                  -rungekutta
#fi

# split up to left and right surfaces
run_cmd ${BINDIR}/off_split ${OUTPUT}_ics.off ${OUTPUT}_ics
run_cmd ${BINDIR}/off_split ${OUTPUT}_ocs.off ${OUTPUT}_ocs
...
run_cmd ${BINDIR}/niikcortex_calc_thickness ${OUTPUT}_ics_${s}.off ${OUTPUT}_ocs_${s}.off ${OUTPUT}_thickness_${hemi}.txt

run_cmd ${BINDIR}/niik_resample_field_sph \
          ${OUTPUT}_thickness_${hemi}.txt \
          ${OUTPUT}_ocs_${s}.off \
          ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.off \
          ${OUTPUT}_thickness_icbm_${hemi}.txt 
        
# convert to ply for easier visualisation
run_cmd ${BINDIR}/off2ply -sph ${icbm_dir}/mni_icbm152_t1_tal_nlin_sym_09c_init_ics_${hemi}.off \
        ${OUTPUT}_thickness_icbm_${hemi}.txt  \
...
echo "  NVK ${img} -obj=${ffn}_ics.off,${ffn}_ocs.off"
