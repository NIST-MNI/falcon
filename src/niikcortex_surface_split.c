...
static int compare_groups(const void *a, const void *b) {
  return (((const struct group_cm *)a)->cm.x - ((const struct group_cm *)b)->cm.x);
}


int split_kobj(kobj *obj, kobj ***out,int verbose ) {
  ...
  for(v=obj->vert,i=0; v!=NULL; v=v->next,i++) {
    ...
    case 0:
      ...
      /* TODO: assert we are not overflowing max_groups ?*/
      ...
      /*      if(group_idx>=max_groups)
            {
              equiv=realloc(2*max_groups*sizeof(int),equiv);
              counts=realloc(2*max_groups*sizeof(int),counts);
              trans=realloc(2*max_groups*sizeof(int),trans);
              memset(equiv+max_groups,max_groups,0);
              memset(counts+max_groups,max_groups,0);
              memset(trans+max_groups,max_groups,0);
              max_groups*=2;
            }*/
    ...
    default:
      ...
      for(c = 0; c < matches; c++) {
        ...
        if(_curr_label != prev_label) {
          ...
          if(equiv[_neighbours[c]] != _min_label) {
            equiv[_neighbours[c]] = _min_label;
          }
        }
  ...
  *out=(kobj**)calloc(sizeof(kobj *),num_groups);
  ...
  if(num_groups>1) {
    ...
    {
      /*sort according to the x,y,z coordiantes */
      /*for sorting groups*/
      ...
      for(v=obj->vert; v!=NULL; v=v->next) {
        ...
        groups_cm[g].cm=niikpt_add(groups_cm[g].cm,v->v);
        groups_cm[g].c++;
      }
      ...
      qsort(groups_cm,sizeof(num_groups),sizeof(groups_cm),compare_groups);
      for(i=0; i<num_groups; i++) {
        trans[groups_cm[i].id+1]=i+1;
      }
...
int main(int argc, char **argv) {
  ...
  NIIK_EXIT( (n=split_kobj( obj, &out_obj, verbose ))==0,      fcname,"split_kobj",1 );
