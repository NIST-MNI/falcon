...
class TmsFemSolution;

// hold parameters and variables of FEM system
class FemSystem
{
public:
    static constexpr unsigned int Dimension = 3;
    static constexpr double scaling_factor = 1e3;
    static constexpr double default_field_value = 0.0;
    static constexpr double grad_epsilon = 0.0; //by default no SVD in gradient calculation

    // ITK setup
    using PixelType = double;
    using ImageType = itk::VectorImage<PixelType, Dimension>;
    using RefImageType = itk::Image<PixelType, Dimension>;
    using BaseImageType = itk::ImageBase<Dimension>;
    ...
    // tetrahedrals
    Eigen::MatrixXi Tet;
    // tags associated with triangles
    Eigen::VectorXi TriTag;
    // tags associated with tetraheras
    Eigen::VectorXi TetTag;

    // node field names (from file)
    std::vector<std::string> XFields;
    // element field names (from file)
    std::vector<std::string> EFields;
    // node fields
    std::vector<Eigen::MatrixXd> XF;
    // triangle element fields
    std::vector<Eigen::MatrixXd> TriF;
    // tetrahedra element fields
    std::vector<Eigen::MatrixXd> TetF;

    // map trianlges to tetrahedrals (i,e each triangle will map to one or two)
    std::unordered_multimap<int, int>      tri_to_tet;
    // map tags to tetrahedras
    std::unordered_multimap<int, int>      tag_to_tet;

    // tetrahedras associated with tag (needed for interpolation on volume)
    std::map<int, Eigen::MatrixXi>          Tet_by_Tag;
    // edge nodes associated with tag (needed for interpolation on volume)
    std::map<int, std::unordered_set<int> > Edge_by_Tag;

    // tetrahedral barycenters
    Eigen::MatrixXd TetBC;
    // tetrahedral volumes
    Eigen::VectorXd vols;

    // element conductiviy 
    Eigen::MatrixXd conductivity;
    ...
    ImageType::Pointer ref_out_image;

    // node coordinates in image index space
    Eigen::MatrixXd C; 

    // needed for interpolation on volume only:
    std::vector<Eigen::Matrix3d> idx_to_bar;

public:
    FemSystem();

    //load mesh definition from file
    void load_mesh(const std::string &mesh_file);

    // setup output volume
    void setup_volumes(BaseImageType * reference=nullptr);

    // prepare for solving (done once per mesh)
    void prepare_mesh(void);

    // run TMS simulation by specifying coil parameters using ITK image
    TmsFemSolution * solve_tms(ImageType::Pointer coil_field, 
        double dIDt, 
        const Eigen::Matrix4d& coil_pos);

    // run TMS simulation by specifying coil dA/dt field
    TmsFemSolution * solve_tms(Eigen::MatrixXd dAdt);

    // find solution for tDCS with given electrode position
    void solve_tdcs(void);
};


// results of one TMS FEM simulation
class TmsFemSolution
{
    public:
    static constexpr unsigned int Dimension = 3;
    ...
    // TMS specific: dA/dt per tetrahedra (D-field from SimNIBS)
    Eigen::MatrixXd dAdt;
    // TMS specific: cond * dA/dt
    Eigen::MatrixXd sigma_dadt;
    // current solution, potential field per node
    Eigen::VectorXd VFieldX;
    // current solution, electric field
    Eigen::MatrixXd EFieldTet;
    // solution relative error: || A*x-b ||/||b||
    double rel_error;

    FemSystem *fem;

    // E-field mapped to volume
    ImageType::Pointer E_volume;

    // normalization volume
    ImageType::Pointer norm_volume;

    TmsFemSolution(FemSystem* sys);

    void calculate_E_field(void);
    void map_E_field_to_volume(void);

    void save_result_mesh(const std::string &fname,bool save_e_field=true, bool save_v_field=false);
    void save_result_image(ImageType::Pointer img,const std::string &fname);
};
