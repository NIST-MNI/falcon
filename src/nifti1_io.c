...
int nifti_io_problem_code_kunio = 0;
...
int nifti_disp_matrix_orient( const char * mesg, mat44 mat ) {
  ...
  if ( mesg ) fputs( mesg, stdout );  /* use stdout? -> use stdout (Kunio 2012-06-02) */
  ...
  /* 2012-06-01, Kunio
   * -changed to stdout from stderr */
  fprintf(stdout, "  i orientation = '%s'\n"
          "  j orientation = '%s'\n"
          "  k orientation = '%s'\n",
...
char *nifti_strdup(const char *str) {
  char *dup= (char *)calloc(strlen(str)+1,1 );
...
char * nifti_findhdrname(const char* fname) {
  ...
  if ( ext && strncmp(ext,".img",4) == 0 ) efirst = 0; /* img */
  else                                     efirst = 1; /* nii */
...
nifti_image* nifti_convert_nhdr2nim(struct nifti_1_header nhdr,
  ...
  if( !is_nifti || nhdr.qform_code <= 0 ) {
    ...
    nim->qto_xyz =
      nifti_quatern_to_mat44(nim->quatern_b, nim->quatern_c, nim->quatern_d,
...
static int need_nhdr_swap( short dim0, int hdrsize ) {
  ...
  volatile short d0    = dim0;     /* so we won't have to swap them on the stack */
  int   hsize = hdrsize;

  if( g_opts.debug > 1 ) fprintf(stdout,"  dim0 = %i,  d0 = %i\n",dim0,d0);
  if( d0 != 0 ) {    /* then use it for the check */
    if( d0 > 0 && d0 <= 7 ) return 0;

    nifti_swap_2bytes(1, (void*)&d0);        /* swap? */
    // if( g_opts.debug > 1) fprintf(stdout,"  dim0 = %i,  d0 = %i   swap\n",dim0,d0);
    if( d0 > 0 && d0 <= 7 ) return 1;
    // if( g_opts.debug > 1 ) fprintf(stdout,"  bad d0,  d0 = %i\n",d0);
...
nifti_image *nifti_image_read( const char *hname, int read_data ) {
  ...
  if( g_opts.debug > 1 ) {
    fprintf(stderr,"-d %s: nifti_is_gzfile = %i \n",fname,nifti_is_gzfile(hfile));
  }
...
static int nifti_read_extensions( nifti_image *nim, znzFile fp, int remain ) {
  ...
  /* fprintf(stdout,"[nifti_read_extensions] start\n"); */
...
void nifti_image_free( nifti_image *nim ) {
  ...
  if( nim->minc_history != NULL )
    free(nim->minc_history) ;
...
znzFile nifti_image_write_hdr_img2(nifti_image *nim, int write_opts,
  ...
  if( ! znz_isnull(imgfile) && nim->nifti_type == NIFTI_FTYPE_NIFTI1_1 ) {
    ...
    /* fprintf(stderr,"+d %i nifti_is_gzfile '%s'\n",nifti_is_gzfile(nim->fname),nim->fname); */
    if( znz_isnull(fp) ) {
      LNI_FERR(func,"cannot open output file",nim->fname);
      nifti_io_problem_code_kunio = 1;
      return fp;
    }
  }
...
znzFile nifti_write_ascii_image(nifti_image *nim, const nifti_brick_list * NBL,
  ...
  if ( write_data   ) {
    nifti_write_all_data(fp,nim,NBL);  /* data */
  }
  if ( ! leave_open ) {
    znzclose(fp);
  }
...
nifti_image * nifti_copy_nim_info(const nifti_image * src) {
  ...
  if( src->minc_history ) dest->minc_history=nifti_strdup(dest->minc_history);
  else dest->minc_history=NULL;
  dest->data = NULL;
