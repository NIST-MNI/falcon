/*
 * this worked:
gcc -o mincexampl1 -I/usr/local/bic/include -L/usr/local/bic/lib -lminc2 -lhdf5 -lnetcdf mincexample1.c
 */
/* ----------------------------- MNI Header -----------------------------------
@NAME       : mincexample1
...
static void get_dimension_info(char *infile, int icvid, Volume *volume) {
  ...
  for (idim=0; idim<ndims; idim++) {
    ...
    switch (idim) {
    case 0:
      dimlength = &(volume->nslices) ;
      break;
    case 1:
      dimlength = &(volume->nrows) ;
      break;
    case 2:
      dimlength = &(volume->ncolumns) ;
      break;
    }
